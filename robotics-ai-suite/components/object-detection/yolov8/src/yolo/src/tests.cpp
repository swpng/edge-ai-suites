...
{
public:
  VideoFrameSubscriber() : Node("video_frame_subscriber")
  {
    video_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
      "video_topic", 10,
      std::bind(&VideoFrameSubscriber::video_callback, this, std::placeholders::_1));
    frame_subscription_ = this->create_subscription<yolo_msgs::msg::YoloFrame>(
      "frame_topic", 10,
      std::bind(&VideoFrameSubscriber::frame_callback, this, std::placeholders::_1));
  }

  // check if size of video_results and frame_results are not empty
  optional<pair<sensor_msgs::msg::Image::SharedPtr, yolo_msgs::msg::YoloFrame::SharedPtr>>
  check_if_finished()
  {
...
template <typename T>
void send_ros2_message(
  typename T::SharedPtr msg, string topic_name, int times = 1, double delay = 0.1)
{
...
enum STATUS { OK, ERR, AWAITING };

map<string, atomic<STATUS>> status_map;

template <typename T>
void expect_ros2_message(
  std::function<bool(T)> check, std::string topic_name, int expected_messages_amount = 1,
  double delay = 0.5)
{
  ...
  threads.push_back(make_shared<thread>([&, topic_name]() {
    ...
    auto subscription =
      node->create_subscription<T>(topic_name, 10, [&](const typename T::SharedPtr msg) {
