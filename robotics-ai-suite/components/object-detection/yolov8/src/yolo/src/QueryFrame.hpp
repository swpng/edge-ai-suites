...
struct QueryFrame
{
  ...
  static cv::Mat draw_detection(
    QueryFrame & frame, unsigned char * data, Task task, float mask_opacity = 0.3)
  {
    cv::Mat image = cv::Mat(frame.msg->height, frame.msg->width, CV_8UC3, data);
    ...
    for (auto & detection : frame.detections) {
      ...
      cv::putText(
        image, label, cv::Point(detection.box.x, detection.box.y - 5), cv::FONT_HERSHEY_SIMPLEX,
        0.5, detection.color, 2);
      if (task == SEGMENTATION) {
        cv::Mat mask_mat =
          cv::Mat(detection.box.height, detection.box.width, CV_32FC1, detection.mask.data());
        cv::Mat mask_mat_8u;
        ...
        for (size_t i = detection.box.y; i < detection.box.y + detection.box.height; i++) {
          for (size_t j = detection.box.x; j < detection.box.x + detection.box.width; j++) {
            if (
              detection.box.contains(cv::Point(j, i)) &&
              detection.mask[(i - detection.box.y) * detection.box.width + (j - detection.box.x)] >
                0.3) {
              image.at<cv::Vec3b>(i, j) =
                image.at<cv::Vec3b>(i, j) * (1.0 - mask_opacity) +
                colored_mask.at<cv::Vec3b>(i - detection.box.y, j - detection.box.x) * mask_opacity;
            }
          }
        }
      }
      if (task == POSE) {
        for (size_t i = 0; i < pose_points; i++) {
          cv::circle(
            image,
            cv::Point(
              detection.pose[i * values_per_point + 1], detection.pose[i * values_per_point + 2]),
            3, detection.color, -1);
        }
        ...
        for (int i = 0; i < SKELETON.size(); i++) {
          if (
            detection.pose[SKELETON[i][0] * values_per_point] > 0.5 &&
            detection.pose[SKELETON[i][1] * values_per_point] > 0.5) {
            cv::line(
              image,
              cv::Point(
                detection.pose[SKELETON[i][0] * values_per_point + 1],
                detection.pose[SKELETON[i][0] * values_per_point + 2]),
              cv::Point(
                detection.pose[SKELETON[i][1] * values_per_point + 1],
