...
int main(int argc, char ** argv)
{
  ...
  try {
    ...
    app.add_option(
      "-p,--performance_mode", performance_mode,
      "Performance mode {1=Latency, 2=Throughput, 3=Cumulative throughput}");
    ...
    std::vector<cv::Size> resolutions = {
      cv::Size(640, 480), cv::Size(320, 240), cv::Size(1280, 720)};
    std::ofstream csv_file;

    try {
      csv_file.open("stats.csv");
      csv_file
        << "resolution,task,model_size,half,device,performance_mode,queue_size,worker_threads,fps_"
           "average,fps_deviation,latency_average,latency_deviation,queue_size_average"
        << std::endl;

      for (Task task : {Task::DETECTION, Task::SEGMENTATION, Task::POSE}) {
        for (ModelSize model_size :
             {ModelSize::N, ModelSize::S, ModelSize::M, ModelSize::L, ModelSize::XL}) {
          for (auto resolution : resolutions) {
            try {
              ...
              std::string run_name = "yolov8" + model_size_strings[(int)model_size] +
                                     task_strs_strings[(int)task] + "_" +
                                     to_string(resolution.width) + "x" +
                                     to_string(resolution.height) + "_" + (half ? "Half" : "Full");
              ...
              pipeline.configure(
                "yolo", {model_path, ""}, device, performance_mode, ov::hint::Priority::HIGH,
                ov::hint::ExecutionMode::PERFORMANCE, std::max(worker_threads / 2, 1),
                worker_threads, "", input_queue_max_size, publish_video, publish_detections);
              ...
              std::string csv_line =
                ...
                task_names_strings[(int)task] + "," + model_size_strings[(int)model_size] + "," +
                std::to_string(half) + "," + device + "," + std::to_string((int)performance_mode) +
                "," + std::to_string(input_queue_max_size) + "," + std::to_string(worker_threads) +
                "," + pipeline.stats.to_csv();
