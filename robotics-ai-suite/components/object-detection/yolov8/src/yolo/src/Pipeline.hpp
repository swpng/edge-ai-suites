...
private:
  ...
  unordered_map<string, rclcpp::Publisher<yolo_msgs::msg::YoloFrame>::SharedPtr>
    detections_publishers;
  ...
  void post_process_callback(
    ov::Tensor & box_proposals, optional<ov::Tensor> & masks_tensor, QueryFrame frame)
  {
    ov::Shape box_proposals_shape;
    ov::Shape masks_shape;

    if (task == DETECTION) {
      box_proposals_shape = ov::Shape({batch_size, 4 + number_of_classes, number_of_boxes});
    }

    if (task == SEGMENTATION) {
      box_proposals_shape =
        ov::Shape({batch_size, 4 + number_of_classes + number_of_masks, number_of_boxes});
      masks_shape = ov::Shape(
        {batch_size, static_cast<unsigned long>(number_of_masks),
    ...
    if (task == POSE) {
      box_proposals_shape =
        ov::Shape({batch_size, 4 + 1 + pose_points * values_per_point, number_of_boxes});
    }
    ...
    move(
      box_proposals.data<float>(), box_proposals.data<float>() + box_proposals.get_size(),
      output_buffer.begin());
    ...
    for (int i = 0; i < b; ++i) {
      for (int j = 0; j < d; ++j) {
        for (int k = 0; k < c; ++k) {
          transposed_output_buffer[i * d * c + j * c + k] = output_buffer[i * c * d + k * d + j];
        }
      }
    }
    ...
    auto process_box = [&](vector<float>::iterator data_start, const int & detection_index) {
      ...
      auto [left, top, width, height] = std::make_tuple(
        int((x - 0.5 * w) * x_factor), int((y - 0.5 * h) * y_factor), int(w * x_factor),
        int(h * y_factor));
      ...
      if (task == SEGMENTATION) {
        ...
        copy(
          data_start + 4 + number_of_classes, data_start + 4 + number_of_classes + number_of_masks,
      ...
      if (task == POSE) {
        ...
        move(
          data_start + 4, data_start + 4 + pose_points * values_per_point,
    ...
    if (task == SEGMENTATION) {
      ...
      for (int i = 0; i < number_of_masks; i++) {
        masks_vector[i] = cv::Mat(
          masks_shape[2], masks_shape[3], CV_32FC1,
          std::move(masks_tensor.value().data<float>() + i * masks_shape[2] * masks_shape[3]));
      }
    }
    ...
    for (size_t i = 0; i < nms_result.size(); ++i) {
      ...
      if (task == SEGMENTATION) {
        cv::Size small_box_size =
          cv::Size((float)result.box.width / x_factor / 4.0f, result.box.height / y_factor / 4);
          ...
          for (int k = 0; k < small_box_size.height; k++) {
            for (int l = 0; l < small_box_size.width; l++) {
              ...
              low_res_mask_mat.at<float>(k, l) +=
                masks_vector[j].at<float>((k + y_offset), (l + x_offset)) * masks[idx][j];
            }
          }
        }
        cv::Mat resized_mask;
        cv::resize(
          low_res_mask_mat, resized_mask, cv::Size(result.box.width, result.box.height), 0, 0,
          cv::INTER_CUBIC);
  ...
  void create_background_thread()
  {
    thread background_thread([&]() {
      ...
      input_buffers.resize(number_of_workers, cv::Mat(4, sizes, ov_element_to_cv_type(input_type)));
      input_datas.resize(number_of_workers);

      auto pre_processing = [&](int i) {
        cv::Size frame_shape = cv::Size(query_frames[i].msg->width, query_frames[i].msg->height);
        cv::Mat input_image = cv::Mat(frame_shape, CV_8UC3, query_frames[i].msg->data.data());

        cv::resize(input_image, input_image, model_shape, 0, 0, cv::INTER_CUBIC);
        cv::dnn::blobFromImage(
          input_image, input_buffers[i], 1.0 / 255.0, model_shape, cv::Scalar(), true, false);
        input_tensors[i] =
      ...
      for (int i = 0; i < number_of_workers; i++) {
        ...
        while (true) {
          if (try_pop) {
            ...
            cout << "Waiting for initial input, Input queue size " << input_queue.size() << endl;
        ...
        while (query_frame_option.has_value()) {
          ...
          if (publish_video) {
            if (video_publishers.find(query_frame.topic_name) == video_publishers.end()) {
              video_publishers[query_frame.topic_name] =
                node->create_publisher<sensor_msgs::msg::Image>(
                  "/" + name + query_frame.topic_name + "/video", 10);
            }
          ...
          if (publish_detections) {
            if (detections_publishers.find(query_frame.topic_name) == detections_publishers.end()) {
              detections_publishers[query_frame.topic_name] =
                node->create_publisher<yolo_msgs::msg::YoloFrame>(
                  "/" + name + query_frame.topic_name + "/yolo_frame", 10);
            }
            detections_publishers[query_frame.topic_name]->publish(
              std::move(*query_frame.to_YoloFrame(task)));
          }
...
public:
  void add_input_topic(string topic_name, int max_fps = -1)
  {
    ...
    auto subscriber = node->create_subscription<sensor_msgs::msg::Image>(
      topic_name, 10,
      [&, topic_name, added_streams_const](const sensor_msgs::msg::Image::SharedPtr image_msg) {
        if (max_fps != -1) {
          auto now = chrono::high_resolution_clock::now();
          auto last_arrival_time = last_arrival_times[added_streams_const];
          auto time_since_last_arrival =
            chrono::duration_cast<chrono::milliseconds>(now - last_arrival_time).count();
  ...
  optional<geometry_msgs::msg::TransformStamped> get_pose(std::string frame)
  {
    ...
    try {
      ...
      RCLCPP_INFO(
        node->get_logger(), "Could not transform %s to %s: %s", map_frame.c_str(), frame.c_str(),
        ex.what());
      return {};
    }
  ...
  void add_joint_rgb_depth_topics(string rgb_topic_name, string depth_topic_name, int max_fps = -1)
  {
    ...
    image_sync[added_streams_const].callback =
      [this, max_fps, rgb_topic_name, added_streams_const](
        ...
        if (
          ...
          cout << "\033[1;31m"
               << "Wrong encoding, had: " << rgb_image->encoding << " " << depth_image->encoding
               << " expected: rgb8 32FC1"
               << "\033[0m" << endl;
  ...
  optional<QueryFrame> try_pop_processed_image()
  {
    ...
    if (query_frame.has_value()) {
      stats.add_latency(query_frame.value().departure_time - query_frame.value().arrival_time);
  ...
  int input_queue_size() { return input_queue.size(); }

  void configure(
    string name, pair<string, string> model_path, string device,
    ...
    size_t num_requests = 2, size_t number_of_workers = 1, string map_frame = "",
    const int input_queue_max_size = 10, bool publish_video = true, bool publish_detections = true)
  {
    ...
    model_shape = cv::Size(
      model->input(0).get_partial_shape().operator[](3).get_length(),
    ...
    if (model->output(0).get_partial_shape().operator[](1).get_length() == 4 + number_of_classes) {
      task = DETECTION;
    } else if (
      model->output(0).get_partial_shape().operator[](1).get_length() ==
      ...
      cout << "\033[1;31m"
           << "Wrong output shape"
           << "\033[0m" << endl;
      cout << "Output shape: " << model->output(0).get_partial_shape().operator[](1).get_length()
           << endl;
    ...
    if (device == "CPU") {
      this->compiled_model = core.compile_model(
        model, device, ov::hint::performance_mode(performance_mode),
        ov::hint::execution_mode(execution_mode), ov::hint::num_requests(num_requests));
    } else {
      this->compiled_model = core.compile_model(
        model, device, ov::hint::performance_mode(performance_mode),
        ov::hint::execution_mode(execution_mode), ov::hint::num_requests(num_requests),
