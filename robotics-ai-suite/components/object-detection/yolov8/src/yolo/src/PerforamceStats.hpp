...
public:
  ...
  void make_latencies_histogram()
  {
    ...
    vector<chrono::duration<double>> latencies =
      get_queue_vector<chrono::duration<double>>(latencies_queue);
    ...
    double sq_sum = inner_product(
      latencies_seconds.begin(), latencies_seconds.end(), latencies_seconds.begin(), 0.0);
    ...
    if (draw_histograms) {
      ...
      matplot::xlabel(
        "Latency (seconds) mean: " + to_string(mean) + " std: " + to_string(stdev) +
        " 99th percentile: " + to_string(percentile_99));
  ...
  void make_fps_histograms()
  {
    ...
    vector<chrono::time_point<chrono::system_clock>> arrival_times =
      get_queue_vector<chrono::time_point<chrono::system_clock>>(arrival_times_queue);
    vector<chrono::time_point<chrono::system_clock>> departure_times =
      get_queue_vector<chrono::time_point<chrono::system_clock>>(departure_times_queue);
    ...
    double sq_sum_arrival =
      inner_product(fps_arrival.begin(), fps_arrival.end(), fps_arrival.begin(), 0.0);
    ...
    double sq_sum_departure =
      inner_product(fps_departure.begin(), fps_departure.end(), fps_departure.begin(), 0.0);
    double stdev_departure =
      sqrt(sq_sum_departure / fps_departure.size() - mean_departure * mean_departure);
  ...
  string to_csv()
  {
    return to_string(measured_fps_average) + "," + to_string(measured_fps_deviation) + "," +
           to_string(measured_latency_average) + "," + to_string(measured_latency_deviation) + "," +
           to_string(measured_queue_size_average);
  }
  ...
  Stats() { update_save_path(save_path); }
};
